...
def validate_attr_type(obj, attr_name, value, attr_type, none_allowed=False, type_name=None):
    ...
    if isinstance(attr_type, (type, tuple)):
        if not isinstance(value, attr_type):
            ...
            raise TypeError(ErrMsg.attr_invalid_type(get_type_name(obj), attr_name, type_name, value))
...
def validate_attr_rgb(obj, attr_name, value, accept_single_num=False, none_allowed=False):
    ...
    if len(value) != 3 or \
        ...
        raise TypeError("trajtracker error: {:}.{:} was set to an invalid value ({:}) - expecting (red,green,blue)".format(get_type_name(obj), attr_name, value))
...
def validate_attr_numeric(obj, attr_name, value, none_value=NoneValues.Invalid):
    if value is None:
        if none_value == NoneValues.Invalid:
            raise TypeError(ErrMsg.attr_invalid_type(get_type_name(obj), attr_name, "numeric", "None"))
    ...
    if value is not None and not isinstance(value, numbers.Number):
        raise TypeError(ErrMsg.attr_invalid_type(get_type_name(obj), attr_name, "numeric", value))
...
def validate_attr_not_negative(obj, attr_name, value):
    if value is not None and value < 0:
        msg = "trajtracker error: {0}.{1} was set to a negative value ({2})".format(get_type_name(obj), attr_name, value)
        raise ValueError(msg)

#--------------------------------------
def validate_attr_positive(obj, attr_name, value):
    if value is not None and value <= 0:
        msg = "trajtracker error: {0}.{1} was set to a negative/0 value ({2})".format(get_type_name(obj), attr_name, value)
        raise ValueError(msg)
